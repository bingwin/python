...
def phone_tv_login(context):
    phone_tv_login = context.driver.find_element_by_android_uiautomator('new UiSelector().resourceId("com.kwai.sogame:id/phone_tv")')
    phone_tv_login.click()

@given(u'登录测试_输入手机号:"{phone}"')
def phone_user(context,phone):
    phone_user = context.driver.find_element_by_android_uiautomator('new UiSelector().resourceId("com.kwai.sogame:id/login_phone_input")')
    time.sleep(1)
    phone_user.click()
    phone_user.clear()
    time.sleep(1)
...
def login_phone_container(context):
    login_phone_container = context.driver.find_element_by_android_uiautomator('new UiSelector().resourceId("com.kwai.sogame:id/login_phone_next")')
    login_phone_container.click()

@given(u'登录测试_输入验证码:"{verification}"')
def login_code_input(context,verification):
    time.sleep(2)
    login_code_input = context.driver.find_element_by_android_uiautomator('new UiSelector().resourceId("com.kwai.sogame:id/login_code_input")')
